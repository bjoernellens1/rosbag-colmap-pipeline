...
def test_quaternion_rotation_roundtrip():
    ...
    R_original = np.array([[0.9999, 0.01, 0.0], [-0.01, 0.9999, 0.01], [0.0, -0.01, 0.9999]])

    # rotation_matrix_to_quaternion returns xyzw (TUM/scipy convention);
    # quaternion_to_rotation_matrix expects COLMAP's wxyz.
    q_xyzw = rotation_matrix_to_quaternion(R_original)
    q_wxyz = np.array([q_xyzw[3], q_xyzw[0], q_xyzw[1], q_xyzw[2]])
    R_recovered = quaternion_to_rotation_matrix(q_wxyz)
...
def test_colmap_pose_to_c2w_rotation():
    ...
    q_w2c = Rotation.from_matrix(R_w2c).as_quat()
    qvec = np.array([q_w2c[3], q_w2c[0], q_w2c[1], q_w2c[2]])  # COLMAP wxyz format
    ...
    expected_R_c2w = Rotation.from_matrix(R_w2c.T).as_matrix()
...
def test_transform_composition():
    ...
    t1 = Transform(rotation=np.eye(3), translation=np.array([1.0, 0.0, 0.0]))

    t2 = Transform(rotation=np.eye(3), translation=np.array([0.0, 1.0, 0.0]))

    t_combined = t1 @ t2

    np.testing.assert_array_almost_equal(t_combined.translation, np.array([1.0, 1.0, 0.0]))
