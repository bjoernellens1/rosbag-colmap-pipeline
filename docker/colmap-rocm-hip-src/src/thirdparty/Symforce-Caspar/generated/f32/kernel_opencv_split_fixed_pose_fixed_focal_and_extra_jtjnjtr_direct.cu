...
__global__ void __launch_bounds__(1024, 1)
    OpencvSplitFixedPoseFixedFocalAndExtraJtjnjtrDirectKernel(
  ...
  if (global_thread_idx < problem_size) {
    ReadShared3<float>((float *)inout_shared,
                       point_njtr_indices_loc[threadIdx.x].target, r0, r1, r2);
  };
  __syncthreads();
  if (global_thread_idx < problem_size) {
  ...
  if (global_thread_idx < problem_size) {
    ReadShared2<float>((float *)inout_shared,
                       principal_point_njtr_indices_loc[threadIdx.x].target, r1,
                       r9);
  };
  __syncthreads();
  if (global_thread_idx < problem_size) {
...
void OpencvSplitFixedPoseFixedFocalAndExtraJtjnjtrDirect(
    float *principal_point_njtr, unsigned int principal_point_njtr_num_alloc,
    SharedIndex *principal_point_njtr_indices, float *principal_point_jac,
    unsigned int principal_point_jac_num_alloc, float *point_njtr,
    unsigned int point_njtr_num_alloc, SharedIndex *point_njtr_indices,
    float *point_jac, unsigned int point_jac_num_alloc,
  ...
  OpencvSplitFixedPoseFixedFocalAndExtraJtjnjtrDirectKernel<<<n_blocks, 1024>>>(
      principal_point_njtr, principal_point_njtr_num_alloc,
