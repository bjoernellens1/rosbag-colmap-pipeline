...
__global__ void __launch_bounds__(1024, 1)
    OpenCVFocalAndExtraUpdateStepFirstKernel(
        float *OpenCVFocalAndExtra_p_kp1,
        unsigned int OpenCVFocalAndExtra_p_kp1_num_alloc,
        const float *const alpha, float *out_OpenCVFocalAndExtra_step_kp1,
  ...
  if (global_thread_idx < problem_size) {
    ...
    WriteIdx4<1024, float, float, float4>(
        ...
        0 * out_OpenCVFocalAndExtra_step_kp1_num_alloc, global_thread_idx, r0,
        r1, r2, r3);
    ...
    WriteIdx2<1024, float, float, float2>(
        out_OpenCVFocalAndExtra_step_kp1,
        4 * out_OpenCVFocalAndExtra_step_kp1_num_alloc, global_thread_idx, r3,
        r4);
  };
}
...
void OpenCVFocalAndExtraUpdateStepFirst(
  ...
  OpenCVFocalAndExtraUpdateStepFirstKernel<<<n_blocks, 1024>>>(
