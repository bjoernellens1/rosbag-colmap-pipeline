...
__global__ void __launch_bounds__(1024, 1) OpenCVFocalAndExtraUpdateStepKernel(
  ...
  if (global_thread_idx < problem_size) {
    ReadIdx4<1024, float, float, float4>(
        OpenCVFocalAndExtra_step_k, 0 * OpenCVFocalAndExtra_step_k_num_alloc,
  ...
  if (global_thread_idx < problem_size) {
    ...
    WriteIdx4<1024, float, float, float4>(
        out_OpenCVFocalAndExtra_step_kp1,
        0 * out_OpenCVFocalAndExtra_step_kp1_num_alloc, global_thread_idx, r4,
        r5, r6, r7);
    ReadIdx2<1024, float, float, float2>(
        OpenCVFocalAndExtra_step_k, 4 * OpenCVFocalAndExtra_step_k_num_alloc,
    ...
    WriteIdx2<1024, float, float, float2>(
        out_OpenCVFocalAndExtra_step_kp1,
        4 * out_OpenCVFocalAndExtra_step_kp1_num_alloc, global_thread_idx, r5,
        r8);
  };
}
...
void OpenCVFocalAndExtraUpdateStep(
  ...
  OpenCVFocalAndExtraUpdateStepKernel<<<n_blocks, 1024>>>(
      OpenCVFocalAndExtra_step_k, OpenCVFocalAndExtra_step_k_num_alloc,
      OpenCVFocalAndExtra_p_kp1, OpenCVFocalAndExtra_p_kp1_num_alloc, alpha,
      out_OpenCVFocalAndExtra_step_kp1,
      out_OpenCVFocalAndExtra_step_kp1_num_alloc, problem_size);
}
