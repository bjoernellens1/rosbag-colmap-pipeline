...
__global__ void __launch_bounds__(1024, 1) OpenCVPrincipalPointUpdateStepKernel(
  ...
  if (global_thread_idx < problem_size) {
    ReadIdx2<1024, float, float, float2>(
        OpenCVPrincipalPoint_step_k, 0 * OpenCVPrincipalPoint_step_k_num_alloc,
  ...
  if (global_thread_idx < problem_size) {
    ...
    WriteIdx2<1024, float, float, float2>(
        out_OpenCVPrincipalPoint_step_kp1,
        0 * out_OpenCVPrincipalPoint_step_kp1_num_alloc, global_thread_idx, r2,
        r4);
  };
}
...
void OpenCVPrincipalPointUpdateStep(
  ...
  OpenCVPrincipalPointUpdateStepKernel<<<n_blocks, 1024>>>(
      OpenCVPrincipalPoint_step_k, OpenCVPrincipalPoint_step_k_num_alloc,
      OpenCVPrincipalPoint_p_kp1, OpenCVPrincipalPoint_p_kp1_num_alloc, alpha,
      out_OpenCVPrincipalPoint_step_kp1,
      out_OpenCVPrincipalPoint_step_kp1_num_alloc, problem_size);
}
