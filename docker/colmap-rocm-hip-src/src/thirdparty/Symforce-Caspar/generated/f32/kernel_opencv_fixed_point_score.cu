...
__global__ void __launch_bounds__(1024, 1) OpencvFixedPointScoreKernel(
  ...
  if (global_thread_idx < problem_size) {
    ReadShared3<float>((float *)inout_shared,
                       pose_indices_loc[threadIdx.x].target, r11, r33, r34);
  };
  __syncthreads();
  if (global_thread_idx < problem_size) {
    r35 = r19 * r21;
...
void OpencvFixedPointScore(float *pose, unsigned int pose_num_alloc,
                           ...
                           unsigned int pixel_num_alloc, float *point,
  ...
  OpencvFixedPointScoreKernel<<<n_blocks, 1024>>>(
