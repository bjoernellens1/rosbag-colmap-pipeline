...
__global__ void __launch_bounds__(1024, 1)
    OpenCVPrincipalPointAlphaNumeratorDenominatorKernel(
  ...
  __shared__ float OpenCVPrincipalPoint_total_ag_local[1];

  __shared__ float OpenCVPrincipalPoint_total_ac_local[1];
  ...
  SumStore<float>(OpenCVPrincipalPoint_total_ag_local, (float *)inout_shared, 0,
  ...
  SumStore<float>(OpenCVPrincipalPoint_total_ac_local, (float *)inout_shared, 0,
                  global_thread_idx < problem_size, r3);
  SumFlushFinal<float>(OpenCVPrincipalPoint_total_ag_local,
                       OpenCVPrincipalPoint_total_ag, 1);
  SumFlushFinal<float>(OpenCVPrincipalPoint_total_ac_local,
                       OpenCVPrincipalPoint_total_ac, 1);
}
...
void OpenCVPrincipalPointAlphaNumeratorDenominator(
  ...
  OpenCVPrincipalPointAlphaNumeratorDenominatorKernel<<<n_blocks, 1024>>>(
      ...
      OpenCVPrincipalPoint_total_ag, OpenCVPrincipalPoint_total_ac,
      problem_size);
}
