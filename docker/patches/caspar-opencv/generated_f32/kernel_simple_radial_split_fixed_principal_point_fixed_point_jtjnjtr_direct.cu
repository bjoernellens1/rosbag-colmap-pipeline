...
__global__ void __launch_bounds__(1024, 1)
  ...
  if (global_thread_idx < problem_size) {
    ...
    WriteSum4<float, float>((float *)inout_shared, r4, r11, r16, r17);
  };
  ...
  if (global_thread_idx < problem_size) {
    ReadIdx4<1024, float, float, float4>(pose_jac, 8 * pose_jac_num_alloc,
                                         global_thread_idx, r17, r16, r11, r4);
    r18 = fmaf(r17, r5, r16 * r10);
    r5 = fmaf(r11, r5, r4 * r10);
    WriteSum2<float, float>((float *)inout_shared, r18, r5);
  };
  ...
  if (global_thread_idx < problem_size) {
    ReadShared2<float>((float *)inout_shared,
                       pose_njtr_indices_loc[threadIdx.x].target, r5, r18);
  };
  __syncthreads();
  if (global_thread_idx < problem_size) {
  ...
  if (global_thread_idx < problem_size) {
    ReadShared4<float>((float *)inout_shared,
                       pose_njtr_indices_loc[threadIdx.x].target, r11, r10, r19,
                       r20);
  };
  __syncthreads();
  if (global_thread_idx < problem_size) {
    ...
    WriteSum2<float, float>((float *)inout_shared, r7, r15);
  };
