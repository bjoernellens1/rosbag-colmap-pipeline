...
__global__ void __launch_bounds__(1024, 1)
  ...
  if (global_thread_idx < problem_size) {
    ...
    ReadIdx4<1024, float, float, float4>(pose_jac, 4 * pose_jac_num_alloc,
                                         global_thread_idx, r22, r23, r24, r25);
    r26 = fmaf(r23, r14, r22 * r21);
    r27 = fmaf(r25, r14, r24 * r21);
    WriteSum4<float, float>((float *)inout_shared, r4, r6, r26, r27);
  };
  ...
  if (global_thread_idx < problem_size) {
    ReadIdx4<1024, float, float, float4>(pose_jac, 8 * pose_jac_num_alloc,
                                         global_thread_idx, r27, r26, r6, r4);
  ...
  if (global_thread_idx < problem_size) {
    ReadShared2<float>((float *)inout_shared,
                       pose_njtr_indices_loc[threadIdx.x].target, r21, r28);
  };
  __syncthreads();
  if (global_thread_idx < problem_size) {
  ...
  if (global_thread_idx < problem_size) {
    ReadShared4<float>((float *)inout_shared,
                       pose_njtr_indices_loc[threadIdx.x].target, r6, r14, r29,
                       r30);
  };
  __syncthreads();
  if (global_thread_idx < problem_size) {
