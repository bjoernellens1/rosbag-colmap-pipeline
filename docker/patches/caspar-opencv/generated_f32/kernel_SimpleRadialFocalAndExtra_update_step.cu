...
__global__ void __launch_bounds__(1024, 1)
    SimpleRadialFocalAndExtraUpdateStepKernel(
  ...
  if (global_thread_idx < problem_size) {
    ReadIdx2<1024, float, float, float2>(
        ...
        0 * SimpleRadialFocalAndExtra_step_k_num_alloc, global_thread_idx, r0,
        r1);
    ReadIdx2<1024, float, float, float2>(
        SimpleRadialFocalAndExtra_p_kp1,
        0 * SimpleRadialFocalAndExtra_p_kp1_num_alloc, global_thread_idx, r2,
        r3);
  };
...
void SimpleRadialFocalAndExtraUpdateStep(
  ...
  SimpleRadialFocalAndExtraUpdateStepKernel<<<n_blocks, 1024>>>(
      SimpleRadialFocalAndExtra_step_k,
