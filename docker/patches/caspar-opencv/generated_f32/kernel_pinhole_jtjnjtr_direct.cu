...
__global__ void __launch_bounds__(1024, 1) PinholeJtjnjtrDirectKernel(
  ...
  __syncthreads();
  if (global_thread_idx < problem_size) {
    ReadIdx2<1024, float, float, float2>(calib_jac, 0 * calib_jac_num_alloc,
                                         global_thread_idx, r18, r19);
    ...
    r25 = fmaf(r23, r17, r22 * r16);
    WriteSum4<float, float>((float *)inout_shared, r4, r6, r24, r25);
  };
  ...
  if (global_thread_idx < problem_size) {
    ReadIdx4<1024, float, float, float4>(pose_jac, 8 * pose_jac_num_alloc,
                                         global_thread_idx, r25, r24, r6, r4);
    r26 = fmaf(r24, r17, r25 * r16);
    r16 = fmaf(r6, r16, r4 * r17);
    WriteSum2<float, float>((float *)inout_shared, r26, r16);
  };
  ...
  if (global_thread_idx < problem_size) {
    ReadShared2<float>((float *)inout_shared,
                       pose_njtr_indices_loc[threadIdx.x].target, r16, r26);
  };
  __syncthreads();
  if (global_thread_idx < problem_size) {
    r25 = fmaf(r16, r25, r26 * r6);
  };
  LoadShared<4, float, float>(pose_njtr, 0 * pose_njtr_num_alloc,
                              pose_njtr_indices_loc, (float *)inout_shared);
  if (global_thread_idx < problem_size) {
    ReadShared4<float>((float *)inout_shared,
                       pose_njtr_indices_loc[threadIdx.x].target, r6, r17, r27,
                       r28);
  };
  __syncthreads();
  if (global_thread_idx < problem_size) {
    ...
    r13 = r25 + r13;
    r18 = r18 * r13;
    r23 = fmaf(r28, r23, r26 * r4);
    r23 = fmaf(r27, r21, r23);
    r23 = fmaf(r6, r1, r23);
    r23 = fmaf(r17, r3, r23);
    r23 = fmaf(r16, r24, r23);
    r5 = r23 + r5;
    r19 = r19 * r5;
    WriteSum4<float, float>((float *)inout_shared, r18, r19, r13, r5);
  };
  ...
  if (global_thread_idx < problem_size) {
    ...
    WriteSum3<float, float>((float *)inout_shared, r10, r12, r15);
  };
  FlushSumShared<3, float>(out_point_njtr, 0 * out_point_njtr_num_alloc,
                           point_njtr_indices_loc, (float *)inout_shared);
}

void PinholeJtjnjtrDirect(
    float *pose_njtr, unsigned int pose_njtr_num_alloc,
    SharedIndex *pose_njtr_indices, float *pose_jac,
    unsigned int pose_jac_num_alloc, float *calib_njtr,
    unsigned int calib_njtr_num_alloc, SharedIndex *calib_njtr_indices,
  ...
  PinholeJtjnjtrDirectKernel<<<n_blocks, 1024>>>(
