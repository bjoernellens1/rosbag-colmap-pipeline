...
__global__ void __launch_bounds__(1024, 1)
    PinholeSplitFixedPoseFixedPrincipalPointResJacFirstKernel(
        ...
        unsigned int pose_num_alloc, float *principal_point,
        unsigned int principal_point_num_alloc, float *out_res,
        unsigned int out_res_num_alloc, float *const out_rTr,
        float *out_focal_jac, unsigned int out_focal_jac_num_alloc,
  ...
  if (global_thread_idx < problem_size) {
    ReadShared2<float>((float *)inout_shared,
                       focal_indices_loc[threadIdx.x].target, r7, r37);
  };
  __syncthreads();
  if (global_thread_idx < problem_size) {
    r42 = r35 + r42;
  ...
  if (global_thread_idx < problem_size) {
    ...
    WriteIdx4<1024, float, float, float4>(out_point_jac,
                                          ...
                                          global_thread_idx, r8, r10, r5, r30);
    r34 = r7 * r46;
...
void PinholeSplitFixedPoseFixedPrincipalPointResJacFirst(
    ...
    unsigned int pose_num_alloc, float *principal_point,
    unsigned int principal_point_num_alloc, float *out_res,
    unsigned int out_res_num_alloc, float *const out_rTr, float *out_focal_jac,
  ...
  PinholeSplitFixedPoseFixedPrincipalPointResJacFirstKernel<<<n_blocks, 1024>>>(
      ...
      out_focal_jac, out_focal_jac_num_alloc, out_focal_njtr,
      out_focal_njtr_num_alloc, out_focal_precond_diag,
      out_focal_precond_diag_num_alloc, out_focal_precond_tril,
      out_focal_precond_tril_num_alloc, out_point_jac, out_point_jac_num_alloc,
