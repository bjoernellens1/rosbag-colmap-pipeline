...
__global__ void __launch_bounds__(1024, 1)
  ...
  __shared__ float SimpleRadialFocalAndExtra_total_ag_local[1];

  __shared__ float SimpleRadialFocalAndExtra_total_ac_local[1];
  ...
  SumStore<float>(SimpleRadialFocalAndExtra_total_ag_local,
                  (float *)inout_shared, 0, global_thread_idx < problem_size,
                  r2);
  if (global_thread_idx < problem_size) {
    ReadIdx2<1024, float, float, float2>(
        SimpleRadialFocalAndExtra_w, 0 * SimpleRadialFocalAndExtra_w_num_alloc,
        global_thread_idx, r2, r3);
    r3 = fmaf(r1, r3, r0 * r2);
  };
  SumStore<float>(SimpleRadialFocalAndExtra_total_ac_local,
                  (float *)inout_shared, 0, global_thread_idx < problem_size,
                  r3);
  SumFlushFinal<float>(SimpleRadialFocalAndExtra_total_ag_local,
                       SimpleRadialFocalAndExtra_total_ag, 1);
  SumFlushFinal<float>(SimpleRadialFocalAndExtra_total_ac_local,
                       SimpleRadialFocalAndExtra_total_ac, 1);
}
...
void SimpleRadialFocalAndExtraAlphaNumeratorDenominator(
  ...
  SimpleRadialFocalAndExtraAlphaNumeratorDenominatorKernel<<<n_blocks, 1024>>>(
      SimpleRadialFocalAndExtra_p_kp1,
      SimpleRadialFocalAndExtra_p_kp1_num_alloc, SimpleRadialFocalAndExtra_r_k,
      SimpleRadialFocalAndExtra_r_k_num_alloc, SimpleRadialFocalAndExtra_w,
      SimpleRadialFocalAndExtra_w_num_alloc, SimpleRadialFocalAndExtra_total_ag,
      SimpleRadialFocalAndExtra_total_ac, problem_size);
}
