...
__global__ void __launch_bounds__(1024, 1)
    SimpleRadialFocalAndExtraUpdateRFirstKernel(
        ...
        const float *const negalpha, float *out_SimpleRadialFocalAndExtra_r_kp1,
  ...
  if (global_thread_idx < problem_size) {
    ReadIdx2<1024, float, float, float2>(
        ...
        0 * SimpleRadialFocalAndExtra_r_k_num_alloc, global_thread_idx, r0, r1);
    ReadIdx2<1024, float, float, float2>(
  ...
  if (global_thread_idx < problem_size) {
    ...
    WriteIdx2<1024, float, float, float2>(
        ...
        0 * out_SimpleRadialFocalAndExtra_r_kp1_num_alloc, global_thread_idx,
        r2, r4);
    r1 = fmaf(r1, r1, r0 * r0);
  };
  ...
  SumFlushFinal<float>(out_SimpleRadialFocalAndExtra_r_0_norm2_tot_local,
                       out_SimpleRadialFocalAndExtra_r_0_norm2_tot, 1);
  SumFlushFinal<float>(out_SimpleRadialFocalAndExtra_r_kp1_norm2_tot_local,
                       out_SimpleRadialFocalAndExtra_r_kp1_norm2_tot, 1);
}
...
void SimpleRadialFocalAndExtraUpdateRFirst(
  ...
  SimpleRadialFocalAndExtraUpdateRFirstKernel<<<n_blocks, 1024>>>(
      ...
      SimpleRadialFocalAndExtra_w, SimpleRadialFocalAndExtra_w_num_alloc,
      negalpha, out_SimpleRadialFocalAndExtra_r_kp1,
      out_SimpleRadialFocalAndExtra_r_kp1_num_alloc,
      out_SimpleRadialFocalAndExtra_r_0_norm2_tot,
      out_SimpleRadialFocalAndExtra_r_kp1_norm2_tot, problem_size);
}
