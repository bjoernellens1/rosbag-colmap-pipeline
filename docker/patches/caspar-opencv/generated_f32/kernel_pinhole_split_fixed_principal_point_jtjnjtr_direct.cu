...
__global__ void __launch_bounds__(1024, 1)
    PinholeSplitFixedPrincipalPointJtjnjtrDirectKernel(
        ...
        unsigned int pose_jac_num_alloc, float *focal_njtr,
        unsigned int focal_njtr_num_alloc, SharedIndex *focal_njtr_indices,
        float *focal_jac, unsigned int focal_jac_num_alloc, float *point_njtr,
        ...
        float *const out_focal_njtr, unsigned int out_focal_njtr_num_alloc,
  ...
  LoadShared<2, float, float>(focal_njtr, 0 * focal_njtr_num_alloc,
                              focal_njtr_indices_loc, (float *)inout_shared);
  if (global_thread_idx < problem_size) {
    ReadShared2<float>((float *)inout_shared,
                       focal_njtr_indices_loc[threadIdx.x].target, r4, r5);
  };
  __syncthreads();
  if (global_thread_idx < problem_size) {
    ReadIdx2<1024, float, float, float2>(focal_jac, 0 * focal_jac_num_alloc,
  ...
  if (global_thread_idx < problem_size) {
    ReadShared3<float>((float *)inout_shared,
                       point_njtr_indices_loc[threadIdx.x].target, r8, r9, r10);
  };
  __syncthreads();
  if (global_thread_idx < problem_size) {
    ...
    r25 = fmaf(r22, r8, r23 * r18);
    WriteSum4<float, float>((float *)inout_shared, r10, r19, r24, r25);
  };
  ...
  if (global_thread_idx < problem_size) {
    ReadIdx4<1024, float, float, float4>(pose_jac, 8 * pose_jac_num_alloc,
                                         global_thread_idx, r25, r24, r19, r10);
    r26 = fmaf(r25, r8, r24 * r18);
    r8 = fmaf(r19, r8, r10 * r18);
    WriteSum2<float, float>((float *)inout_shared, r26, r8);
  };
  ...
  if (global_thread_idx < problem_size) {
    ReadShared2<float>((float *)inout_shared,
                       pose_njtr_indices_loc[threadIdx.x].target, r8, r26);
  };
  __syncthreads();
  if (global_thread_idx < problem_size) {
    r25 = fmaf(r8, r25, r26 * r19);
  };
  LoadShared<4, float, float>(pose_njtr, 0 * pose_njtr_num_alloc,
                              pose_njtr_indices_loc, (float *)inout_shared);
  if (global_thread_idx < problem_size) {
    ReadShared4<float>((float *)inout_shared,
                       pose_njtr_indices_loc[threadIdx.x].target, r19, r18, r27,
                       r28);
  };
  __syncthreads();
  if (global_thread_idx < problem_size) {
    r25 = fmaf(r27, r20, r25);
    r25 = fmaf(r28, r22, r25);
    r25 = fmaf(r19, r0, r25);
    r25 = fmaf(r18, r2, r25);
    r9 = r25 + r9;
    r9 = r6 * r9;
    r23 = fmaf(r28, r23, r26 * r10);
    ...
    r17 = r23 + r17;
    r17 = r7 * r17;
    WriteSum2<float, float>((float *)inout_shared, r9, r17);
  };
  ...
  if (global_thread_idx < problem_size) {
    r25 = r4 + r25;
    r23 = r5 + r23;
    r14 = fmaf(r14, r23, r13 * r25);
    r16 = fmaf(r16, r23, r15 * r25);
    r23 = fmaf(r12, r23, r11 * r25);
    WriteSum3<float, float>((float *)inout_shared, r14, r16, r23);
  };
...
void PinholeSplitFixedPrincipalPointJtjnjtrDirect(
  ...
  PinholeSplitFixedPrincipalPointJtjnjtrDirectKernel<<<n_blocks, 1024>>>(
      ...
      pose_jac_num_alloc, focal_njtr, focal_njtr_num_alloc, focal_njtr_indices,
      focal_jac, focal_jac_num_alloc, point_njtr, point_njtr_num_alloc,
      point_njtr_indices, point_jac, point_jac_num_alloc, out_pose_njtr,
      out_pose_njtr_num_alloc, out_focal_njtr, out_focal_njtr_num_alloc,
