...
__global__ void __launch_bounds__(1024, 1)
    SimpleRadialFocalAndExtraAlphaDenominatorOrBetaNumeratorKernel(
        ...
        float *const SimpleRadialFocalAndExtra_out, size_t problem_size) {
  ...
  SumStore<float>(SimpleRadialFocalAndExtra_out_local, (float *)inout_shared, 0,
                  global_thread_idx < problem_size, r3);
  SumFlushFinal<float>(SimpleRadialFocalAndExtra_out_local,
                       SimpleRadialFocalAndExtra_out, 1);
}
...
void SimpleRadialFocalAndExtraAlphaDenominatorOrBetaNumerator(
  ...
  SimpleRadialFocalAndExtraAlphaDenominatorOrBetaNumeratorKernel<<<n_blocks,
                                                                   1024>>>(
      SimpleRadialFocalAndExtra_p_kp1,
      SimpleRadialFocalAndExtra_p_kp1_num_alloc, SimpleRadialFocalAndExtra_w,
      SimpleRadialFocalAndExtra_w_num_alloc, SimpleRadialFocalAndExtra_out,
      problem_size);
}
