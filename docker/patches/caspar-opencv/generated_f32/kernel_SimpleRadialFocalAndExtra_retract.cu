...
__global__ void __launch_bounds__(1024, 1)
  ...
  if (global_thread_idx < problem_size) {
    ReadIdx2<1024, float, float, float2>(
        SimpleRadialFocalAndExtra, 0 * SimpleRadialFocalAndExtra_num_alloc,
...
void SimpleRadialFocalAndExtraRetract(
  ...
  SimpleRadialFocalAndExtraRetractKernel<<<n_blocks, 1024>>>(
      SimpleRadialFocalAndExtra, SimpleRadialFocalAndExtra_num_alloc, delta,
      delta_num_alloc, out_SimpleRadialFocalAndExtra_retracted,
      out_SimpleRadialFocalAndExtra_retracted_num_alloc, problem_size);
}
