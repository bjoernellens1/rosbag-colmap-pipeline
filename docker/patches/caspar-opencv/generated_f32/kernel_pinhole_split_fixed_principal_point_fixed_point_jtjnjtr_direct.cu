...
__global__ void __launch_bounds__(1024, 1)
    PinholeSplitFixedPrincipalPointFixedPointJtjnjtrDirectKernel(
        ...
        float *focal_jac, unsigned int focal_jac_num_alloc,
  ...
  if (global_thread_idx < problem_size) {
    ReadIdx4<1024, float, float, float4>(pose_jac, 8 * pose_jac_num_alloc,
                                         global_thread_idx, r15, r14, r9, r8);
  ...
  if (global_thread_idx < problem_size) {
    ReadShared2<float>((float *)inout_shared,
                       pose_njtr_indices_loc[threadIdx.x].target, r5, r16);
  };
  __syncthreads();
  if (global_thread_idx < problem_size) {
  ...
  if (global_thread_idx < problem_size) {
    ReadShared4<float>((float *)inout_shared,
                       pose_njtr_indices_loc[threadIdx.x].target, r9, r4, r17,
                       r18);
  };
  __syncthreads();
  if (global_thread_idx < problem_size) {
...
void PinholeSplitFixedPrincipalPointFixedPointJtjnjtrDirect(
  ...
  PinholeSplitFixedPrincipalPointFixedPointJtjnjtrDirectKernel<<<n_blocks,
      ...
      focal_jac, focal_jac_num_alloc, out_pose_njtr, out_pose_njtr_num_alloc,
      out_focal_njtr, out_focal_njtr_num_alloc, problem_size);
}
