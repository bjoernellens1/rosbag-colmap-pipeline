...
__global__ void __launch_bounds__(1024, 1)
    SimpleRadialSplitFixedFocalAndExtraJtjnjtrDirectKernel(
        ...
        unsigned int principal_point_jac_num_alloc, float *point_njtr,
  ...
  if (global_thread_idx < problem_size) {
    ReadShared3<float>((float *)inout_shared,
                       point_njtr_indices_loc[threadIdx.x].target, r6, r7, r8);
  };
  __syncthreads();
  if (global_thread_idx < problem_size) {
  ...
  if (global_thread_idx < problem_size) {
    ReadIdx4<1024, float, float, float4>(pose_jac, 8 * pose_jac_num_alloc,
                                         global_thread_idx, r23, r22, r17, r8);
  ...
  if (global_thread_idx < problem_size) {
    ReadShared4<float>((float *)inout_shared,
                       pose_njtr_indices_loc[threadIdx.x].target, r17, r16, r25,
                       r26);
  };
  __syncthreads();
  if (global_thread_idx < problem_size) {
...
void SimpleRadialSplitFixedFocalAndExtraJtjnjtrDirect(
    float *pose_njtr, unsigned int pose_njtr_num_alloc,
    SharedIndex *pose_njtr_indices, float *pose_jac,
    unsigned int pose_jac_num_alloc, float *principal_point_njtr,
    unsigned int principal_point_njtr_num_alloc,
  ...
  SimpleRadialSplitFixedFocalAndExtraJtjnjtrDirectKernel<<<n_blocks, 1024>>>(
      ...
      principal_point_jac_num_alloc, point_njtr, point_njtr_num_alloc,
      point_njtr_indices, point_jac, point_jac_num_alloc, out_pose_njtr,
      out_pose_njtr_num_alloc, out_principal_point_njtr,
      out_principal_point_njtr_num_alloc, out_point_njtr,
